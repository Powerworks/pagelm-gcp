...
export const Ragsearch: ToolIO = {
  ...
  schema: {
    type: 'object',
    properties: { q: { type: 'string' }, ns: { type: 'string' }, k: { type: 'number' } },
    required: [],
  },
  run: async (input: any, ctx: Record<string, any>) => {
    ...
    const kNum = Number(input?.k ?? 6);
    const k = Number.isFinite(kNum) && kNum > 0 ? Math.min(kNum, 20) : 6;
    if (!q) return [{ text: '' }];
    const retriever = await getRetriever(ns, embeddings);
    const docs = await retriever.invoke(q);
    const out = (docs || [])
      .slice(0, k)
      .map((d: any) => ({ text: toStr(d?.pageContent || ''), meta: d?.metadata || {} }));
