  ...
  app.post('/smartnotes', async (req: any, res: any) => {
    try {
      ...
      if (!topic && !notes && !filePath) {
        return res.status(400).send({ ok: false, error: 'Provide topic, notes, or filePath' });
      }

      const noteId = crypto.randomUUID();
      nlog('start', noteId, 'input:', { topic, notes, filePath });

      res.status(202).send({ ok: true, noteId, stream: `/ws/smartnotes?noteId=${noteId}` });

      setImmediate(async () => {
        try {
          emitToAll(ns.get(noteId), { type: 'phase', value: 'generating' });
          const result = await withTimeout(handleSmartNotes({ topic, notes, filePath }), 120000, 'handleSmartNotes');
          nlog('generated', noteId, result.file);
          emitToAll(ns.get(noteId), {
            type: 'file',
            file: `${config.url}/storage/smartnotes/${path.basename(result.file)}`,
          });
