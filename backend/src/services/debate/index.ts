...
export async function addDebateMessage(sessionId: string, role: 'user' | 'assistant', content: string): Promise<void> {
...
export async function* streamDebateAnalysis(
  sessionId: string
): AsyncGenerator<{ type: 'phase'; value: string } | { type: 'analysis'; data: DebateAnalysis }, void, unknown> {
  const session = await getDebateSession(sessionId);
  if (!session) {
    throw new Error('Debate session not found');
  }
...
Provide a comprehensive analysis in this EXACT JSON format (no markdown, just JSON):
{
  ...
  const messages = [
    {
      ...
      content: 'You are an expert debate judge. Provide fair, balanced analysis. Respond with valid JSON only.',
    },
  ...
  try {
    ...
    const fallbackAnalysis: DebateAnalysis = {
      ...
      reason:
        session.winner === 'user'
          ? 'User presented stronger arguments'
          : session.winner === 'ai'
            ? 'AI presented stronger arguments'
            : 'Both sides presented balanced arguments',
  ...
  try {
    ...
    return {
      ...
      overallAssessment:
        'The debate featured ' +
        userMessages.length +
        ' exchanges with arguments from both sides. Due to technical limitations, a detailed analysis could not be completed, but both participants engaged meaningfully in the discussion.',
    };
  }
}
