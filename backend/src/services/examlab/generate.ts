...
import type { ExamPayload, ExamSpec, QuizLikeItem } from './types';
  ...
  for (const sec of s.payload.sections) {
    ...
    for (const it of sec.items) {
      ...
      if (!it.question || !Array.isArray(it.options) || it.options.length !== 4)
        throw new Error(`bad item in ${sec.id}`);
      if (typeof it.correct !== 'number' || it.correct < 1 || it.correct > 4)
        throw new Error(`bad correct in ${sec.id}`);
