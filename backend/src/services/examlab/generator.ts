...
import type { GenSpec, QuizLikeItem } from './types';
...
const SYS = (count: number, style?: string, difficulty?: string, topic?: string) =>
  `PRIMARY OBJECTIVE
...
function cleanOptions(v: any) {
  const o = splitOptions(v).map(s => s.replace(/^\s*(?:[A-D]\)|[1-4]\))\s*/i, '').trim());
...
function coerce(items: any, count: number): QuizItem[] {
  ...
  while (out.length < count) {
    ...
    const src = out[i - 1] || {
      ...
      correct: 1,
      hint: 'Use the core idea.',
      explanation: 'The correct option matches the main idea; others do not.',
    };
...
function validItem(x: any) {
  return (
    ...
    x.options.length === 4 &&
    x.options.every((o: any) => typeof o === 'string') &&
  ...
  if (gen.type === 'mcq') {
    ...
    const key = keyOf(
      JSON.stringify({
        ...
        topic: normalizeTopic(gen.topic),
        prompt: promptNorm,
      })
    );
