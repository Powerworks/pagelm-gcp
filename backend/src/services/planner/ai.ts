import type { Task, TaskType, Slot, PlanPolicy } from './types';
...
function heuristicParse(text: string): Partial<Task> {
  ...
  const timePatterns = [/~(\d+)h/i, /~(\d+)\s*hours?/i, /(\d+)m/i, /(\d+)\s*mins?/i];
...
function parseDateHeuristic(dateStr: string): string {
  ...
  let targetDay = weekdays.findIndex(day => day === lowerStr);
  if (targetDay === -1) {
...
export function makeSlots(tasks: Task[], policy: PlanPolicy): Slot[] {
  const sortedTasks = [...tasks].sort((a, b) => calculateUrgencyScore(b) - calculateUrgencyScore(a));
...
function findNextAvailableSlot(
  ...
  const searchTime = new Date(fromTime);
  ...
  for (let day = 0; day < 14; day++) {
    ...
    const currentSlot = new Date(Math.max(dayStart.getTime(), searchTime.getTime()));
...
export function replan(missedSlots: Slot[], remainingSlots: Slot[], tasks: Task[], policy: PlanPolicy): Slot[] {
  const affectedTaskIds = new Set([...missedSlots.map(s => s.taskId), ...remainingSlots.map(s => s.taskId)]);
  ...
  return [...unaffectedSlots, ...newSlots].sort((a, b) => new Date(a.start).getTime() - new Date(b.start).getTime());
}
