...
function server() {
  ...
  const SERVER = http.createServer((req, res) => {
    ...
    let i = 0;
    let next = () => {
      if (i < fns.length) fns[i++](req, res, next);
    };
    next();
  });
  ...
  const matchRoute = (a, b) => {
    for (let i = 0; i < ROUTES.length; i++) {
      let r = ROUTES[i];
      if (r.method !== a && r.method !== 'ALL') continue;
  ...
  const add = (a, b, c) => {
    ROUTES.push({ method: a.toUpperCase(), path: b, handler: c });
  };
  const use = a => {
    WARES.push(a);
  };
  ...
  const getRoutes = () =>
    ROUTES.reduce((acc, { method, path }) => ((acc[method] = acc[method] || []).push(path), acc), {});
  const serverStatic = (endpoint, dir) => {
    ...
    return function staticMiddleware(req, res, next) {
      ...
      if (!(d && !d.startsWith('..') && !path.isAbsolute(d))) return next();
      fs.stat(c, (err, stats) => {
