...
export default function BagDrawer({ open, items, onClose, onClear }: Props) {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50" onClick={onClose}>
      <div
        className="absolute right-4 top-4 bottom-4 w-96 bg-stone-950 border border-stone-900 rounded-2xl p-6 overflow-hidden flex flex-col"
      ...
      >
        ...
        <div className="mt-4 pt-4 border-t border-stone-900">
          <button
            onClick={onClear}
            className="w-full bg-red-900/20 hover:bg-red-900/30 border border-red-800 text-red-400 rounded-xl px-4 py-2 text-sm transition-colors"
          >
            Clear All Items
          </button>
        </div>
