  ...
  return (
    <div id="quizContent" className="space-y-8">
      <div id="questionCard" className="bg-stone-950 border border-stone-900 rounded-2xl p-6">
        <div className="mb-6">
          <h2 id="questionText" className="text-xl font-semibold text-white mb-4">
            {q.question}
          </h2>
          <div
            id="questionImage"
            className={`${q.imageHtml ? '' : 'hidden'} mb-4`}
          ...
          {q.options.map((opt, i) => {
            ...
            return (
              <div
                ...
                className={cls}
                onClick={() => onSelect(i)}
                style={{ pointerEvents: showExp ? 'none' : 'auto' }}
              >
        ...
        <div className="mt-6 flex flex-col md:flex-row justify-between items-center">
          <button
            onClick={onHint}
            className="bg-stone-900 hover:bg-stone-800 border border-stone-900 hover:border-stone-800 text-stone-200 hover:text-white rounded-xl px-4 py-2 font-medium transition-all duration-300 flex items-center gap-2"
          >
