...
export default function Sidebar() {
  ...
  const b = (t: string) =>
    `p-2 rounded-xl duration-300 transition-all ${p === t ? 'text-white bg-stone-800' : 'hover:bg-stone-900 hover:text-stone-200'}`;
  ...
  return (
    <aside className="left-0 bottom-0 w-screen md:w-fit md:h-screen fixed p-4 z-50 lg:flex">
      <div
        id="idk"
        className="w-64 md:order-2 h-full z-40 rounded-l-none rounded-2xl bg-stone-950 border border-l-transparent border-stone-900 hidden flex-col p-4 space-y-3 overflow-y-auto custom-scroll"
      >
        {chats?.chats.map(chat => (
          <Link
            key={chat.id}
            to={`/chat/?chatId=${chat.id}`}
            className="p-2 hover:text-stone-200 hover:bg-stone-900 w-full rounded-xl block text-sm min-h-fit truncate"
          >
      ...
      <div
        id="idk0"
        className="md:w-20 md:order-1 h-full rounded-3xl bg-stone-950/50 backdrop-blur-xl border border-stone-900 text-stone-400 flex flex-col items-center justify-between py-4"
      >
