        ...
        {slots.map(s => (
          <div key={s.id} className="flex items-center justify-between text-sm text-stone-200/90">
            ...
            <div className="text-stone-400 text-xs">
              {fmtTime(s.start)} → {new Date(s.end).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
            </div>
          </div>
        ))}
      </div>
...
export default function Planner() {
  ...
  const [loadingStates, setLoadingStates] = useState<
    Record<string, { plan?: boolean; summary?: boolean; flashcards?: boolean }>
  >({});
  ...
  useEffect(() => {
    wsRef.current = connectPlannerStream(sid, (ev: PlannerEvent) => {
      if (ev.type === 'plan.update') {
        setTasks(t =>
          t.map(x => (x.id === ev.taskId ? ({ ...x, plan: { ...(x as any).plan, slots: ev.slots } } as any) : x))
        );
        plannerWeekly(false)
          .then(wp => setPlan(wp.plan))
          .catch(() => {});
      }
      ...
      if (ev.type === 'task.file.removed') {
        setTasks(t =>
          t.map(x => (x.id === ev.taskId ? { ...x, files: (x.files || []).filter(f => f.id !== ev.fileId) } : x))
        );
      }
        ...
        <div className="flex items-center gap-2">
          <div className="text-xs bg-zinc-900 border border-zinc-800 rounded overflow-hidden">
            <button
              ...
              className={`px-2 py-1 ${view === 'today' ? 'bg-zinc-800 text-zinc-100' : 'text-zinc-300'}`}
            >
              Today
            </button>
            <button
              onClick={() => setView('list')}
              className={`px-2 py-1 ${view === 'list' ? 'bg-zinc-800 text-zinc-100' : 'text-zinc-300'}`}
            >
              List
            </button>
            <button
              onClick={() => setView('mindmap')}
              className={`px-2 py-1 ${view === 'mindmap' ? 'bg-zinc-800 text-zinc-100' : 'text-zinc-300'}`}
            >
              Mindmap
            </button>
          </div>
          <button onClick={reload} className="text-xs px-2 py-1 rounded bg-stone-800 text-stone-200">
            Refresh
          </button>
        </div>
      </div>
              ...
              <div key={t.id} className="rounded-xl border border-zinc-800 bg-stone-950 p-3">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <select
                      value={t.status}
                      onChange={e => mark(t.id, e.target.value as any)}
                      className="bg-stone-900 border border-zinc-800 text-stone-200 text-xs rounded px-2 py-1"
                    >
                    ...
                    <button
                      onClick={() => planTask(t.id)}
                      className="text-xs px-2 py-1 rounded bg-zinc-800 text-zinc-200"
                    >
                      Plan
                    </button>
                    <button
                      onClick={() => gen(t.id, 'summary')}
                      className="text-xs px-2 py-1 rounded bg-zinc-800 text-zinc-200"
                    >
                      Summary
                    </button>
                    <button
                      onClick={() => gen(t.id, 'flashcards')}
                      className="text-xs px-2 py-1 rounded bg-zinc-800 text-zinc-200"
                    >
                      Flashcards
                    </button>
                    <button onClick={() => del(t.id)} className="text-xs px-2 py-1 rounded bg-red-600 text-white">
                      Delete
                    </button>
                  </div>
                </div>
                  ...
                  <div className="mt-3 border-t border-zinc-800 pt-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {t.files.map(file => (
                        <div
                          key={file.id}
                          className="flex items-center gap-1 px-2 py-1 bg-zinc-800 rounded text-xs text-zinc-200"
                        >
