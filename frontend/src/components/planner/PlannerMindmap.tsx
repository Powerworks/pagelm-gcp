...
export default function PlannerMindmap({
  tasks,
  plan,
  onPlan,
  onAssist,
  ...
  useEffect(() => {
    try {
      localStorage.setItem('planner.customNodes', JSON.stringify(customNodes));
    } catch {}
  }, [customNodes]);
      ...
      for (const t of tasks) {
        ...
        try {
          ...
          if (Array.isArray(data.importantTerms))
            labels.push(...data.importantTerms.map((x: any) => x?.term).filter(Boolean));
  ...
  useEffect(() => {
    for (const id of Object.keys(simsRef.current)) {
      try {
        simsRef.current[id].stop();
      } catch {}
    }
    simsRef.current = {};
  ...
  useEffect(() => {
    try {
      localStorage.setItem('planner.stepPos', JSON.stringify(stepPos));
    } catch {}
  }, [stepPos]);
  ...
  return (
    ...
    >
            ...
            <g key={t.id}>
              {labels.map(label => {
                ...
                const {
                  ...
                  bx: bxWorld,
                  by: byWorld,
                } = anchorLine(cxWorld, cyWorld, rxC, ryC, sxWorld, syWorld);
      ...
      {tasks.map(t => {
        ...
        if (!p) return null;
        const labels = aiSteps[t.id]?.length ? aiSteps[t.id]! : inferSteps(t).map(s => s.label);
        const counts = stepCountsFor(
          t,
          labels.map(l => ({ label: l, w: 1 }))
        );
        return (
          <div
            key={t.id}
            ...
            data-node
            style={{ left: pan.x + p.x * zoom, top: pan.y + p.y * zoom }}
          >
            <div style={{ transform: `scale(${zoom})`, transformOrigin: 'top left', position: 'relative' }}>
              ...
              >
                <div
                  className="pointer-events-none absolute -inset-2 opacity-35"
                  style={{ background: `radial-gradient(22px 22px at 50% 65%, rgba(56,189,248,0.2), transparent 70%)` }}
                />
                <div
                  onPointerDown={e => onDown(e, t.id)}
                  className="cursor-grab active:cursor-grabbing rounded-full border bg-[#0b1220]/70 border-[#1f2937] pl-3 pr-1.5 py-1.5 shadow-[0_2px_8px_rgba(0,0,0,0.4)] backdrop-blur flex items-center gap-1.5"
                >
                  ...
                  <button
                    ...
                    className="text-[12px] text-slate-300 hover:text-slate-100 px-1 rounded hover:bg-stone-800"
                  >
                    ⋯
                  </button>
                </div>
                {menuOpen === t.id && (
                  <div className="absolute left-0 top-[120%] z-30 bg-stone-900/95 border border-zinc-800 rounded-md shadow-lg min-w-[160px] p-1">
                    <button
                      onClick={() => {
                        setMenuOpen(null);
                        onUpdateStatus(t.id, 'doing');
                        onPlan(t.id);
                      }}
                      className="w-full text-left text-[12px] text-stone-200 px-2 py-1 rounded hover:bg-stone-800"
                    >
                      Start now
                    </button>
                    <button
                    ...
                    <button
                      onClick={() => {
                        const val = window.prompt('Add note');
                        if (val != null) {
                          onUpdateNotes?.(t.id, val);
                        }
                        setMenuOpen(null);
                      }}
                      className="w-full text-left text-[12px] text-stone-200 px-2 py-1 rounded hover:bg-stone-800"
                    ...
                    <button
                      ...
                      className="w-full text-left text-[12px] text-red-400 px-2 py-1 rounded hover:bg-red-950/40"
                    >
                      Delete task
                    </button>
                  </div>
              ...
              {labels.map(label => {
                ...
                return (
                  <div
                    key={sid}
                    className="absolute rounded-full border border-zinc-800 bg-stone-950/80 text-center text-stone-300 cursor-grab active:cursor-grabbing"
                    onPointerDown={e => {
                      const rect = wrapRef.current?.getBoundingClientRect();
                      const localX = rect ? e.clientX - rect.left : e.clientX;
                      const localY = rect ? e.clientY - rect.top : e.clientY;
                  ...
                  >
                    ...
                    <button
                      onClick={e => {
                        ...
                        setAiSteps(s => {
                          const labels =
                            s[t.id] && Array.isArray(s[t.id])
                              ? s[t.id].slice()
                              : aiSteps[t.id]?.slice() || inferSteps(t).map(x => x.label);
                          const nextLabels = labels.filter(l => l !== label);
                          const next = { ...s, [t.id]: nextLabels };
                          try {
                            localStorage.setItem(`planner.aiSteps.${t.id}`, JSON.stringify(nextLabels));
                          } catch {}
                          return next;
                        });
                        setStepPos(sp => {
              ...
              <div className="flex items-center gap-2">
                {editingId === n.id ? (
                  <input
                    ...
                    onBlur={e => {
                      setCustomNodes(list => list.map(x => (x.id === n.id ? { ...x, label: e.target.value } : x)));
                      setEditingId(null);
                    }}
                    className="bg-transparent text-[12px] text-slate-200 outline-none"
                  />
                ) : (
                  <div className="text-[12px] text-slate-200" onDoubleClick={() => setEditingId(n.id)}>
                    {n.label}
                  </div>
                )}
                <button
                  onClick={() => setCustomNodes(list => list.filter(x => x.id !== n.id))}
      ...
      <div className="absolute top-3 right-3 z-20 bg-stone-900/70 backdrop-blur border border-zinc-800 rounded-lg px-2 py-1 flex items-center gap-2 text-xs text-stone-300">
        <button
          ...
          className="px-2 py-1 bg-stone-800 rounded"
        >
          Add bubble
        </button>
        <button
