...
export function buildTaskSim(
  taskId: string,
  cx: number,
  cy: number,
  steps: Array<{ id: string; x: number; y: number; label: string }>,
  ...
  for (const s of steps)
    nodes.push({ id: `s:${s.id}`, x: s.x + 42, y: s.y + 14, type: 'step', taskId, label: s.label });
  ...
  sim.on('tick', () => {
    ...
    for (const n of nodes)
      if (n.type === 'step' && n.taskId && typeof n.x === 'number' && typeof n.y === 'number')
        out[`${n.taskId}::${n.label}`] = { x: n.x - 42, y: n.y - 14 };
