...
export type QuizEvent = {
  type: 'ready' | 'phase' | 'quiz' | 'done' | 'error' | 'ping';
  quizId?: string;
  value?: string;
  quiz?: unknown;
  error?: string;
...
async function req<T = unknown>(url: string, init: RequestInit & { timeout?: number } = {}): O<T> {
...
export async function chatAskOnce(opts: {
  ...
  await new Promise<void>((resolve, reject) => {
    const { close } = connectChatStream(start.chatId, ev => {
      ...
      if (ev.type === 'error') {
        close();
        reject(new Error(ev.error || 'chat failed'));
      }
    });
  });
...
export async function createFlashcard(input: { question: string; answer: string; tag: string }) {
...
export async function getExams() {
  return req<{ ok: true; exams: { id: string; name: string; sections: any[] }[] }>(`${env.backend}/exams`, {
    method: 'GET',
  });
}

export async function startExam(examId: string) {
  return req<{ ok: true; runId: string; stream: string }>(`${env.backend}/exam`, {
    method: 'POST',
    headers: jsonHeaders({}),
...
export type PlannerEvent =
  ...
  | {
      type: 'daily.digest';
      date: string;
      due: { id: string; title: string; dueAt: number }[];
      sessions: number;
      message: string;
    }
  | {
      ...
      date: string;
      stats: any;
      tomorrowTasks: { id: string; title: string }[];
      message: string;
    }
  ...
  | {
      ...
      session: { id: string; taskId: string; slotId?: string; startedAt: string; status: string };
    }
  | {
      type: 'session.ended';
      session: { id: string; endedAt: string; minutesWorked: number; completed: boolean; status: string };
    }
...
export async function plannerPlan(id: string, cram?: boolean) {
  return req<{ ok: boolean; task: PlannerTask & { plan?: { slots: PlannerSlot[] } } }>(
...
export async function submitDebateArgument(debateId: string, argument: string) {
  return req<{ ok: boolean; message: string; error?: string }>(
...
export async function getDebateSession(debateId: string) {
  return req<{ ok: boolean; session: DebateSession; error?: string }>(
...
export async function analyzeDebate(debateId: string) {
  return req<{ ok: boolean; analysis: DebateAnalysis; session: DebateSession; error?: string }>(
