...
export default function Chat() {
      ...
      (async () => {
        try {
          ...
          if (list.length) {
            ...
            if (latest?.id) {
              setChatId(latest.id);
              navigate(`/chat?chatId=${encodeURIComponent(latest.id)}`, {
                replace: true,
                state: { chatId: latest.id },
              });
            } else {
  ...
  useEffect(() => {
    ...
    if (state.answer) {
      ...
      if (init.flashcards?.length || state.flashcards?.length)
        setCards(init.flashcards?.length ? init.flashcards : state.flashcards || []);
      if (init.topic) setTopic(init.topic || '');
      else if (init.md) setTopic(deriveTopicFromMarkdown(init.md));
    } else {
      if (initialQuestion) {
        setMessages(prev =>
          Array.isArray(prev) && prev.length ? prev : [{ role: 'user', content: initialQuestion, at: Date.now() }]
        );
  ...
  useEffect(() => {
    if (!chatId) return;
    const wsUrl =
      (env.backend || window.location.origin).replace(/^http/, 'ws') + `/ws/chat?chatId=${encodeURIComponent(chatId)}`;
  ...
  useEffect(() => {
    const onSel = () => {
      ...
      setSelected({
        ...
        x: rect.left + rect.width / 2 - 60 + window.scrollX,
        y: rect.bottom + window.scrollY,
      });
    };
    const onDocClick = (e: any) => {
      const n = e.target as Node;
      if (selPopupRef.current && !selPopupRef.current.contains(n) && !window.getSelection()?.toString().trim())
        setSelected(null);
    };
  ...
  const addToBag = async (kind: BagItem['kind'], title: string, content: string) => {
    ...
    try {
      ...
      setBag(b => [{ id: flashcard.id, kind, title: flashcard.question, content: flashcard.answer }, ...b]);
  ...
  return (
    <div className="flex flex-col min-h-screen w-full px-4 lg:pl-28 lg:pr-4">
        ...
        <div className="flex-1 pr-6">
          <div className="w-full max-w-5xl mx-auto p-4 pt-2 pb-28">
            <div className="space-y-6">
              {list.map((m, i) => {
                const userBubble =
                  'inline-block max-w-[85%] bg-stone-900/70 border border-zinc-800 rounded-2xl px-4 py-3';
                if (m.role === 'assistant') {
                  return (
            ...
            {latestAssistantContent && !awaitingAnswer && (
              <ActionRow
                disabled={busy}
                onSummarize={() =>
                  sendFollowup('Summarize the previous answer into 5–7 concise bullet points with bolded keywords.')
                }
                onLearnMore={() =>
                  sendFollowup(
                    'Go deeper into this topic with advanced details, real-world examples, and a short analogy.'
                  )
                }
                onStartQuiz={() => {
      ...
      <SelectionPopup
        ...
        addNote={text => {
          addToBag('note', `Note: ${text.slice(0, 30)}${text.length > 30 ? '...' : ''}`, text);
          setSelected(null);
        }}
        askDoubt={text => {
          const v = text.trim();
