...
export default function Debate() {
  ...
  const [debateId, setDebateId] = useState<string | null>(searchParams.get('debateId'));
  ...
  const submitArgument = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/debate/${debateId}/argue`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleSurrender = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/debate/${debateId}/surrender`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      });
  ...
  const fetchAnalysis = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/debate/${debateId}/analyze`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      });
      ...
      <div className="flex flex-col items-center justify-center min-h-screen w-full px-4 lg:pl-28 lg:pr-4 bg-black">
        <div className="max-w-2xl w-full space-y-8">
          <div className="text-center space-y-4">
            <h1 className="text-4xl lg:text-5xl font-bold text-white">Start a Debate</h1>
            <p className="text-lg text-stone-400">Challenge AI in a structured debate on any topic</p>
          </div>

          <div className="bg-stone-950/90 backdrop-blur-sm border border-stone-900 rounded-2xl p-8 space-y-6">
            <div>
              <label className="block text-sm font-medium text-stone-300 mb-2">Debate Topic</label>
              <input
                type="text"
                value={topic}
            ...
            <div>
              <label className="block text-sm font-medium text-stone-300 mb-3">Your Position</label>
        ...
        {messages.length === 0 && !isStreaming && (
          <div className="flex items-center justify-center h-full">
            <div className="text-center space-y-4 max-w-md">
              ...
              <h2 className="text-2xl font-bold text-white animate-[fadeIn_0.6s_ease-in-out]">Ready to debate!</h2>
              <p className="text-stone-400 animate-[fadeIn_0.7s_ease-in-out]">
                Present your opening argument below. The AI will respond with a counterargument.
              </p>
            </div>
          </div>
        )}
        ...
        {messages.map((msg, idx) => (
          ...
          >
            <div className={`max-w-3xl ${msg.role === 'user' ? 'items-end' : 'items-start'} flex flex-col gap-2`}>
              ...
              <div
                className={`px-6 py-4 rounded-2xl shadow-lg transition-all hover:shadow-xl ${
                  msg.role === 'user'
                    ? session?.position === 'for'
        ...
        <div className="max-w-4xl mx-auto">
          <div className="flex gap-3 mb-3">
            ...
            <button
              ...
              className={`px-6 py-3 rounded-xl font-semibold transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-lg ${
                session?.position === 'for'
            ...
            <div className="p-8 space-y-6">
              ...
              <div className="text-center space-y-4">
                ...
                <h2 className="text-3xl font-bold text-white">
                  {analysis.winner === 'user' ? 'You Won!' : analysis.winner === 'ai' ? 'AI Won!' : "It's a Draw!"}
                </h2>
