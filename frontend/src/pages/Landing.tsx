  ...
  const onSend = async (override?: string) => {
    ...
    try {
      ...
      adaptiveToast.error(
        ...
        'There was a problem reaching the AI. Please try sending your prompt again.'
      );
    } finally {
      setBusy(false);
    }
        ...
        <div className="w-full md:w-fit flex">
          <div className="w-full md:w-fit md:min-w-fit p-1.5 rounded-2xl rounded-t-none bg-stone-950 flex flex-col items-start sm:items-center border border-stone-900 border-t-0 border-r-0 border-b-0 sm:border-b shadow-[inset_2px_-2px_15px] shadow-stone-900/80">
            <div className="flex items-start justify-between md:justify-start space-x-2 p-1.5 w-full">
                  ...
                  <div className="absolute top-full left-0 mt-1 p-1 w-36 bg-stone-950 border border-stone-800 rounded-xl shadow-lg z-20">
                    {['Chat', 'Quiz'].map(opt => (
                      <div
                        ...
                        className={`px-3 py-2 cursor-pointer hover:bg-stone-800 transition rounded-lg ${
                          mode === opt ? 'text-sky-400' : 'text-white'
                        }`}
                      >
                        {opt}
                      </div>
              ...
              <div
                className="p-1.5 rounded-xl hover:bg-white/5 duration-300 transition-all cursor-pointer min-w-fit h-fit hidden md:flex items-center space-x-4"
                onClick={onPickFile}
                title={stagedFile ? stagedFile.name : 'Click or drop'}
              >
                <div className="flex flex-col -space-y-0.5 pr-2">
                  <span className="text-xs text-stone-300">{stagedFile ? 'File selected' : 'Add files'}</span>
                  <span className="text-sm font-semibold text-white">
                    {stagedFile ? stagedFile.name : 'Click or drop'}
                  </span>
                </div>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  ...
                  <div className="absolute top-full left-0 mt-1 p-1 w-40 bg-stone-950 border border-stone-800 rounded-xl shadow-lg z-20">
                    {['Short', 'Medium', 'Long'].map(opt => (
                      <div
                        ...
                        className={`px-3 py-2 cursor-pointer hover:bg-stone-800 transition rounded-lg ${
                          responseLength === opt ? 'text-sky-400' : 'text-white'
                        }`}
                      >
                        {opt}
                      </div>
          ...
          <svg
            className="h-9 w-8 -ml-0.5 -mt-[2px] min-w-fit hidden md:block rotate-3"
            viewBox="0 0 72 72"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              d="M1 1H71C21.4764 5.44502 6.18653 20.4467 1 71V1Z"
              fill="#101110"
              strokeWidth="2"
              stroke="url(#paint0_linear_1409_7)"
            />
            <defs>
