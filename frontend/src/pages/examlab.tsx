...
export default function ExamLabs() {
  ...
  const resultVisual = useMemo(() => {
    if (percentage >= 90)
      return { msg: 'Excellent!', cls: 'bg-green-900/20 border border-green-700 text-green-200', icon: '🏆' };
    if (percentage >= 70)
      return { msg: 'Great job!', cls: 'bg-blue-900/20 border border-blue-700 text-blue-200', icon: '🎉' };
    if (percentage >= 50)
      return { msg: 'Good effort!', cls: 'bg-yellow-900/20 border border-yellow-700 text-yellow-200', icon: '📚' };
            ...
            {exams.map(ex => (
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <div className="text-lg font-semibold text-white">{ex.name}</div>
                  <div className="px-3 py-1 rounded-full bg-sky-900/30 text-sky-300 text-xs font-medium">
                    {ex.sections.length} sections
                  </div>
                </div>
                <div className="text-sm text-stone-400">Click to start this exam</div>
              </div>
            ))}
          </div>

          {!loadingExams && !exams.length && (
            <div className="mt-16 text-center text-stone-400">No exams available.</div>
          )}
        </div>
      </div>
    );
